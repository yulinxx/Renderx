    ...
    namespace core
    {
        ...
        void MeshManager::uploadInstanceBuffer(rhi::IDevice* device)
        {
            ...
            for (uint32_t i = 0; i < m_instances.size(); ++i)
            {
                ...
                instanceData[i].color[0] = m_instances[i].color[0];
                instanceData[i].color[1] = m_instances[i].color[1];
                instanceData[i].color[2] = m_instances[i].color[2];
                instanceData[i].color[3] = m_instances[i].color[3];
            ...
            const float projMatrix[16])
        {
            ...
            for (uint32_t i = 0; i < visibleCount; ++i)
            {
                ...
                d.color[0] = inst.color[0];
                d.color[1] = inst.color[1];
                d.color[2] = inst.color[2];
                d.color[3] = inst.color[3];
                d.flags = inst.flags;
            ...
            device->setUniformMatrix4("uProjMatrix", projMatrix);
            float ambient[3] = { 0.25f, 0.25f, 0.25f };
            float specular[3] = { 0.4f, 0.4f, 0.4f };
            device->setUniformVec3("uAmbientColor", ambient);
            device->setUniformVec3("uSpecularColor", specular);
            device->setUniformFloat("uShininess", 32.0f);
