...
namespace render
{
    ...
    {
        const float* vertices;   ///< 顶点位置数组（每顶点3个float: x,y,z）；当 usePerVertexColor=true 且 colors=nullptr
                                 ///< 时，解释为 OverlayVertex*（7 floats per vertex，自带 RGBA）
        uint32_t vertexCount;    ///< 顶点数量
        bool usePerVertexColor;  ///< 是否使用逐顶点颜色（为true且colors有效时读float
                                 ///< RGB；为true且colors为空时读OverlayVertex）
    ...
    struct DrawCommand
    {
        ...
        // 类型需在匿名 union 外声明，避免 -Wnested-anon-types 警告
        struct WorldData
        {
